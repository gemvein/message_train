  ...
  class MessagesController < NightTrain::ApplicationController
    ...
    def create
      ...
      if @box.errors.all.empty?
        if @message.draft
          redirect_to night_train.box_path(:drafts), alert: :message_saved_as_draft.l
        else
          redirect_to night_train.box_path(:sent), notice: :message_sent.l
        end
      else
    ...
    def update
      ...
      if @box.errors.all.empty?
        if @message.draft
          redirect_to night_train.box_conversation_url(@box, @message.conversation), alert: :message_saved_as_draft.l
        else
          redirect_to night_train.box_path(:sent), notice: :message_sent.l
        end
      else
  ...
  private
    ...
    def message_params
      params.require(:message).permit(
          ...
          recipients_to_save: NightTrain.configuration.recipient_tables
      )
    end
  end
end
